#!/usr/bin/env python3
"""
merge_macros.py - v3.4.0
- REMOVED: All pre-action jitter and micro-mouse movements.
- FEATURE: Random micro-delay (0-100ms) per event index for timing variance.
- FEATURE: Idle Mouse Movements for gaps > 5s (Simulates waiting/breathing).
- FIX: Resolved TypeError in mouse movement calculation by sanitizing None coordinates.
- STABLE: Linear timeline reconstruction with randomized micro-pacing.
"""
...
def main():
    ...
    parser.add_argument("--delay-before-action-ms", type=int, default=100) # Max random range
    ...
    for d in ["originals", "input_macros"]:
        ...
        if test_path.exists() and test_path.is_dir():
            ...
            break
    if not originals_root: originals_root = search_base
    
    logout_file = None
    for loc in [originals_root / "logout.json", originals_root.parent / "logout.json", search_base / "logout.json"]:
        if loc.exists():
            logout_file = loc
    ...
    for root, dirs, files in os.walk(originals_root):
        ...
        is_z = "z +100" in str(curr).lower()
        parent_scope = next((p for p in curr.parts if p.lower() in ["desktop", "mobile"]), None)
        
        if is_z and parent_scope:
    ...
    for k, data in pools.items():
        ...
        manifest = [f"MANIFEST: {data['rel']}", "="*30]
        ...
        for v_idx in range(1, (norm_v + inef_v) + 1):
            ...
            mult = rng.choice([1.0, 1.2, 1.5]) if data["is_ts"] else (rng.choices([1, 2, 3], weights=[20, 40, 40] if is_inef else [50, 30, 20])[0])
            ...
            for i, p in enumerate(paths):
                ...
                for e_idx, e in enumerate(raw):
                    ...
                    # Random interval between 0 and the input delay (default 100ms)
            ...
            manifest.append(f"Version {v_code}: {format_ms_precise(timeline)} (Mult x{mult})")

        (out_f / f"!_MANIFEST_{data['num']}_!.txt").write_text("\n".join(manifest))
